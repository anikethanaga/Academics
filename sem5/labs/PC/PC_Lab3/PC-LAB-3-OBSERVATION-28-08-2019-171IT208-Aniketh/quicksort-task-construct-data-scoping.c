...
int main()
{
	...
	printf("Time taken for serial approach = %lf microseconds\n\n",time*1000000);
	
	time = omp_get_wtime();
	quicksort_task(B,0,n-1);
	time = omp_get_wtime()-time;
	printf("Time taken for for program using task construct = %lf microseconds\n\n",time*1000000);


	time = omp_get_wtime();
	quicksort_task_wait(C,0,n-1);
	time = omp_get_wtime()-time;
	printf("Time taken for for program using taskwait and data scoping = %lf microseconds\n\n",time*1000000);
